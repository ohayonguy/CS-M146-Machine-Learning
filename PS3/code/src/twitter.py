...
def extract_feature_vectors(infile, word_list):
    ...
    with open(infile, 'rU') as fid :
        ...
        for line_index, line in enumerate(fid):
            words = extract_words(line)
            for word_index, key in enumerate(word_list):
                if key in words:
                    feature_matrix[line_index][word_index] = 1
        ### ========== TODO : END ========== ###
...
def performance(y_true, y_pred, metric="accuracy"):
    ...
    if metric == 'accuracy':
        ...
        return roc_auc_score(y_true, y_label)
...
def cv_performance(clf, X, y, kf, metric="accuracy"):
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        avg_performance += performance(y_test, y_pred, metric=metric)

    return avg_performance / kf.get_n_splits()
    ### ========== TODO : END ========== ###
...
def select_param_linear(X, y, kf, metric="accuracy"):
    ...
    for c in C_range:
        print('C: {}'.format(c))
        result = cv_performance(SVC(kernel='linear', C=c), X, y, kf=kf, metric=metric)
        print('result: {0:.4f}\n'.format(result))
...
def select_param_rbf(X, y, kf, metric="accuracy"):
    ...
    gamma_range = np.arange(0.001, 0.002, 0.001)
    ...
    for c in C_range:
        for gamma in gamma_range:
            ...
            result = cv_performance(SVC(kernel='rbf', C=c, gamma=gamma), X, y, kf=kf, metric=metric)
            #print('result: {0:.4f}\n'.format(result))
            ...
            if best_performance is None:
                ...
                best_C = c
                best_gamma = gamma
            elif result > best_performance + 0.001:
                best_performance = result
                best_C = c
                best_gamma = gamma
    ...
    print('Best Score: {0:.4f}'.format(best_performance))
...
def main() :
    ...
    rbf_svm = SVC(kernel='rbf', C=60, gamma=0.006)
    linear_svm.fit(X_train, y_train)
    rbf_svm.fit(X_train, y_train)
    for metric in metric_list:
        print("rbf:")
        performance_test(rbf_svm, X_test, y_test, metric=metric)
        print("linear:")
        performance_test(linear_svm, X_test, y_test, metric=metric)
        print("\n")
