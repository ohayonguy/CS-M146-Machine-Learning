...
class Cluster(object) :
    ...
    def centroid(self) :
        ...
        center = np.array([p.attrs for p in self.points])
        label_count = {}
        for p in self.points:
        ...
        centroid = Point('centroid', most_common_label, np.mean(center, axis=0))
        return centroid
        ### ========== TODO : END ========== ###
    ...
    def medoid(self) :
        ...
        # part 2b: implement
        medoid = None
        medoid_avg_dist = np.inf
        for point in self.points:
            sum_of_distances = 0
            for other_point in self.points:
                sum_of_distances += point.distance(other_point)
            if sum_of_distances < medoid_avg_dist:
                medoid_avg_dist = sum_of_distances
                medoid = point
        return medoid
        ### ========== TODO : END ========== ###
...
class ClusterSet(object):
    ...
    def add_point_to_cluster(self, cluster_index, point):
        self.members[cluster_index].add(point)
