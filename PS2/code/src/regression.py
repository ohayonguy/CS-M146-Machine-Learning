...
class PolynomialRegression() :
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in xrange(tmax) :
            ...
            grad = 2 * np.sum((y_pred - y).reshape(-1,1)*X, axis=0)
...
def main() :
    ...
    learning_rates = [0.0001, 0.001, 0.01, 0.0407]
    coeffs = []
    iters = []
    last_errs = []
    times = []
    ...
    for lr_rate in learning_rates:
        ...
        model, num_iter, _ = model.fit_GD(X_train, y_train, eta=lr_rate)
        times.append(time.time()-time_begin)
        coeffs.append(model.coef_)
        iters.append(num_iter)
        last_errs.append(model.cost(X_train, y_train))
    ...
    model = PolynomialRegression()
    model.coef_ = np.zeros(2)
    _, _, fit_GD_time = model.fit_GD(X_train, y_train)

    print 'Changing eta GD solution:'
    print model.coef_
    print 'Fit time for closed form:'
    print fit_time
    print 'Fit time for GD:'
    print fit_GD_time
    ...
    model = PolynomialRegression()
    model.coef_ = np.zeros(2)
    _, _, fit_GD_time = model.fit_GD(X_train, y_train, eta=0.0407)
    print fit_GD_time
    ### ========== TODO : END ========== ###
    ...
    plt.plot(list(range(11)), RMSE_train)
    plt.plot(list(range(11)), RMSE_test)
    plt.legend(['RMSE on training data','RMSE on test data'])
    plt.xlabel('m')
    plt.show()
    ### ========== TODO : END ========== ###
